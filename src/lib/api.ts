...
export const auth = {
  ...
  async signIn(email:string, password:string) {
    ...
    if (!cleanEmail || !password) throw makeError('Enter your email and password.');
    const { data, error } = await supabase.auth.signInWithPassword({ email:cleanEmail, password });
    if (error) throw makeError(error.message,401);
    if(!data.user||!data.session) throw makeError('Could not create a signed-in session.',401);
    await ensureProfile();
  ...
  async signUp(email:string, password:string, fullName:string) {
    ...
    if (!cleanEmail || !password) throw makeError('Enter your email and password.');
    if (password.length < 8) throw makeError('Password must be at least 8 characters.');
    const { data, error } = await supabase.auth.signUp({email:cleanEmail,password,options:{data:{full_name:name}}});
    if (error) throw makeError(error.message,400);
    if (!data.user) throw makeError('Could not create the account.',400);
    if (!data.session) throw makeError('Account created. Check your email to confirm the account, then sign in.',403);
    await ensureProfile();
    return {user:{userId:data.user.id,email:data.user.email||undefined,name:data.user.user_metadata?.full_name||name},accessToken:data.session.access_token,expiresIn:3600};
  },
